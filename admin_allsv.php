...
<html>
    ...
    <body>
        ...
        <div class="btn-group">
            ...
            <button type="button" onclick="location.href='admin_allsv.php'" class="btn btn-secondary active" style="font-size: 25px;">小衛糾</button>
            <button type="button" onclick="location.href='admin_allmiss.php'" class="btn btn-secondary" style="font-size: 25px;">今日缺勤</button>
        </div>
        <br><!--名單統整之衛糾-->
        <form action="sv_delete.php" method="post">
        <?php
            require('connect_mysql.php');
            $sql_query="SELECT * FROM sv order by sv_studynum";
			...
			while($row=mysqli_fetch_assoc($result)){
				echo "<tbody>
                <tr>
				<td><font size='5'>".$row["sv_studynum"]."</font></td>
				<td><font size='5'>".$row["sv_class"]."</font></td>
				<td><font size='5'>".$row["sv_classnum"]."</font></td>
				<td><font size='5'>".$row["sv_name"]."</font></td>
                <td><font size='5'>".$row["sv_work"]."</font></td>
                <td><i class='fas fa-trash-alt'><a href='sv_delete.php?sv_studynum=".$row['sv_studynum']."'><input method='POST' type='button' id='sv_delete' name='sv_delete' class='btn'></a></i></td>
				</tr>
                </tbody>";
			}
			echo"</table>";
